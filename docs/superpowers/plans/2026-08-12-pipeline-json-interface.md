...
- Produces: `driver.approve_review(stem, review: dict) -> dict` — result `{"stem", "state": "approved", "fingerprint"}`.
  ...
  - **Single snapshot:** load `rec` once; compute `provenance.review_revision(stem, rec)`; when `expected_review_revision` present and different → `CommandError("STALE_REVIEW", …)`; when present, also require `provenance.stale_styles(stem, rec) == []` → else `STALE_REVIEW` listing the styles. When absent, skip both gates (spec §4.2 compatibility scoping) — "absent" covers a review-file that omits the field as well as every legacy `approve <stem>` invocation, which never reaches this path at all. Unconditional gating is deliberately rejected: `provenance.stale_styles` reports missing provenance as stale, so it would make every recipe predating provenance recording permanently un-approvable.
