...
**Date:** 2026-08-12
**Status:** Draft for review (rev 5 — post Codex review round 4, loop closed)
...
- **Pipeline changes are additive only.** All existing CLI invocations behave as today when the new flags are absent, with exactly two declared exceptions: (a) mutating commands gain lock acquisition (below); (b) preview-generating paths additionally record optional provenance/dimension fields in recipes (new keys only — no existing key's value changes, no state transition changes). The existing test suite (171 tests) keeps passing; any existing test that asserts exact recipe key sets is updated for (b) explicitly, never silently.
...
| `RepoWatcher` | FSEvents on `Input/`, `previews/`, `sidecars/`, `recipes/`, `config/` (styles, RT seed, toolchain lock — they're review-revision inputs), `Output/`, and `run/` (lock transitions); coalesces bursts (500 ms) and triggers a `status --json` refresh. While `lock.held` by an external process, additionally polls `status` every 5 s so a no-op CLI run can't leave the busy pill stuck. | FSEvents |
...
| Command | Semantics |
|---|---|
| `status --json` | Read-only, lock-free, side-effect-free snapshot (§4.3). **Requires a non-persisting manifest read path**: today `manifest.load()` rebuilds *and saves* `.manifest`, which would make `status` an unlocked mutator racing locked commands — `status` gets a read-only reconstruction; persistence stays inside locked mutating commands; a test asserts `status` changes no repo files. Reports a crash-stale lock (PID not alive) as `held: false, stale: true` without deleting it. Snapshot coherence: after assembling the result, `status` re-stats the recipe/manifest files it read and retries once (100 ms) if any changed mid-read; residual cross-file staleness is acceptable because every mutation ends in an FSEvents-triggered refresh that converges the UI. |
| `ingest [--from PATH ...] [--delivery-id ID] --json` | Existing ingest of `Input/`, plus: `--from` copies external files/folders into `Input/` itself — each source is first copied to a private temp file, **the temp is hashed** (never the live source, so a source changing mid-copy can't invalidate the decision), dedup/conflict decisions are made from the staged hash (content duplicate → skip; same-stem-different-hash → reject, nothing placed), then atomic rename into `Input/`, all under the lock, so a concurrent CLI ingest can never see a half-copied RAW. `--delivery-id` (app-supplied UUID) and an `ingested_at` (UTC RFC 3339, microseconds) are recorded in the new photos' recipes **only when the flag is passed** — flag-less CLI ingest produces byte-identical legacy recipes. |
| `preview --stem S --style Y --json` | Re-render one preview JPG. Renders to a temp file, atomically replaces the target only on success (failure leaves the previous JPG intact). Records in the recipe, atomically with the decoded render dimensions: per-style **preview provenance** — the hash of the rendering-input material the preview was produced from, plus the produced JPG's content hash. A style is *stale* (`status.stale_previews`) if **either** check fails: stored input hash ≠ current rendering inputs, **or** stored content hash ≠ the JPG actually on disk (a swapped preview file is stale even when inputs never moved). Revision-carrying approvals (the app's) refuse while any style is stale — this is what makes "the pixels you reviewed are the pixels that render" provable rather than assumed for app-driven approvals; legacy approvals are unaffected (§4.2 approve row). The batch preview path records the same provenance (a declared additive exception in §2). Without `--stem/--style`, `preview` otherwise behaves exactly as today. |
| `adjust --stem S --style Y [--temperature K] [--exposure EV] [--reset] --json` | Merges values into `sidecars/S_Y.pp3` — `[White Balance]` `Setting=Custom`, `Temperature`, `Green=1.0` (matching the operator contract); `[Exposure]` `Compensation` — preserving all unrelated sections/keys; atomic write; then re-renders that preview. Ownership tracking in the recipe: `app_adjustments[style][control] = {previous, last_written}`, where a *control* is an atomic bundle — white balance owns the coupled `Setting`/`Temperature`/`Green` triple as one unit (captured, divergence-checked, and restored all-or-none), exposure owns `Compensation`. `--reset` restores an owned control to `previous` (removing its keys, and the file if nothing else remains, when there was none) **only if every key in the bundle still equals `last_written`**; any divergence (hand edit after the app wrote it) leaves the whole bundle untouched and drops ownership. Sidecar is written before the recipe; on the next command, an owned key whose value ≠ `last_written` is reconciled as hand-edited (ownership dropped), so a crash between the two writes can't misattribute edits. Only flags actually passed are written. |
...
| `approve --stem S --review-file P --json` | Reads audit entries + crop windows from the JSON file (§4.3). **Single-snapshot rule:** approve reads every input file exactly once, computes the review revision *and* the approval fingerprint from those same bytes — an edit landing between check and persist can't enter the fingerprint. **Validates both supplied windows** via `geometry.validate_crop` against the recipe's recorded render dimensions, and requires every style's preview provenance to match current inputs (no stale previews), *before* persisting anything. `expected_review_revision` mismatch or stale previews → `STALE_REVIEW`, nothing changes. **Compatibility scoping:** the revision and stale-preview gates apply only when the review-file carries `expected_review_revision` (the app always sends it) — a review-file without it, and every legacy approve invocation, keeps today's exact semantics, so recipes with no recorded provenance remain approvable via CLI and the existing tests are untouched. The "provable pixels" guarantee is therefore a property of app-driven approvals; CLI approvals retain the CLI's existing trust model. |
| `run [--stem S] [--force] --json` | Plain `run` keeps its **exact legacy semantics** — ingested photos get previews and stop where today's state machine stops them (`preview_ready`/`review_required`), fingerprint demotion remains a successful needs-review stop, batch previews record dimensions as they do today. The UI maps any pre-approval stop state to "needs review". New forms only: `--stem` restricts to one photo; `--force` re-renders even from rendered/verified (the Reprocess menu). An `adjust` after approval stales the fingerprint exactly as a hand edit does today; the affected photo simply returns to needs-review rather than erroring. |
...
{"ok": false, "error": {"code": "PARTIAL_FAILURE", "message": "1 of 3 photos failed"}, "result": { ... }}
```

`result` **may accompany `ok: false`** — aggregate commands (`run`, `ingest`) always include it so successful publications/ingests aren't hidden by one failure; the app processes `result` first, then surfaces the error. Error codes (closed set, growable): `LOCK_HELD`, `TOOLCHAIN_FAILED`, `RENDER_FAILED`, `VERIFY_FAILED`, `INVALID_STATE`, `STALE_REVIEW`, `PARTIAL_FAILURE`, `NOT_FOUND`, `BAD_INPUT`, `INTERNAL`.

**`review_revision`** (used by `status`, `adjust`/`preview` results, and `approve`; one shared helper): sha256 over the photo's **approval-fingerprint material** — the same canonical inputs the existing fingerprint helper hashes (recipe content, raw hash, base style profiles, sidecars, RT seed, toolchain lock entries) — **plus each preview JPG's content hash (or absence marker)**. Content hashes, not mtimes, so a same-mtime replacement can't slip through, and a `config/` style edit moves the revision just like a sidecar edit. Approval can never bind rendering inputs or previews the reviewer didn't see.
...
- `ingest`: `{"ingested": ["P1036171"], "skipped": [{"file": "P1036163.RW2", "reason": "duplicate content"}], "conflicts": [{"file": "...", "reason": "stem exists with different content"}], "failed": [{"file": "...", "code": "BAD_INPUT", "message": "unreadable RW2"}]}` — skips/conflicts alone are `ok: true`; per-file ingest failures that let other photos continue produce `ok: false` + `PARTIAL_FAILURE` with the full `result` attached.
- `preview` / `adjust`: `{"stem": "...", "style": "...", "preview": "previews/..._preview.jpg", "temperature": {"value": 5700, "source": "sidecar"}, "exposure": {"value": 0.12, "source": "sidecar"}, "review_revision_before": "sha256:…", "review_revision_after": "sha256:…"}` (effective post-merge values; the revision pair drives draft rebasing, §6.1).
...
- `run`: `{"published": [{"stem": "...", "version": "v004", "artifact_count": 29}], "advanced": [{"stem": "...", "state": "preview_ready"}], "failed": [{"stem": "...", "code": "VERIFY_FAILED", "message": "..."}]}` — `ok: true` iff `failed` is empty; otherwise `PARTIAL_FAILURE` with this `result` attached (successes still published).
- `status`:
  ...
  "photos": [
    {
      ...
      "review_revision": "sha256:…",               // approval-fingerprint material + preview content hashes (§4.3)
      "previews": {"natural": "previews/P1036163_natural_preview.jpg", "...": "..."},
      "preview_hashes": {"natural": "sha256:…", "...": "..."},   // canvas cache key (§6)
      "stale_previews": [],                                       // styles whose preview provenance ≠ current inputs
...
3. **Slider:** UI value → debounce → `adjust` (lock, sidecar merge, preview re-render all pipeline-side) → envelope → watcher event → canvas reloads image, **cache keyed by the preview's content hash from `status`** (never mtime or URL caching — a same-mtime replacement must still repaint). A photo with non-empty `stale_previews` shows a "preview out of date — re-render" chip per affected style (one click issues `preview --stem --style`); Approve is disabled while any style is stale (and `approve` enforces the same pipeline-side).
...
A draft (audit checkboxes, note, crop nudges — slider values are *not* drafts; they commit to disk via `adjust` on debounce) is transient UI state keyed to the photo's `review_revision` from the `status` snapshot it was started against. The revision covers all approval-fingerprint material plus preview content (§4.3), so it moves on *any* input change — a CLI sidecar edit, a `config/` style edit, a replaced preview. **Rebase rule:** after the app's own successful `adjust`/`preview`, the draft rebases from the command's `review_revision_before`/`after` pair — only if the draft's key equals `before` and the refreshed state equals `after`. While one of the app's own commands for a stem is in flight, staleness evaluation for that stem is **deferred** — watcher refreshes during the command's normal intermediate states (sidecar written, preview not yet replaced) don't flap the draft; reconciliation happens once, at the envelope + terminal refresh. Any other movement (including an interleaved external edit hiding behind the app's own command, or a failed `adjust` that changed the sidecar but not the preview) marks the draft **stale** — contents preserved, banner shown ("This photo changed on disk — re-check before approving"), Approve disabled until the user re-confirms the checklist against the fresh state. `approve` independently enforces the same thing pipeline-side via `expected_review_revision` → `STALE_REVIEW`, so a race that slips past the UI still can't approve unseen pixels. Drafts are dropped on app quit by design.
...
- **Pipeline (pytest, added to the existing suite):** envelope shape + exit-code rule incl. `PARTIAL_FAILURE`-with-result on aggregate commands; progress-event format and 1-based/stage-scoped counting; `status --json` schema round-tripping real repo states (empty, ingested, review_required, published, legacy recipe without delivery fields, stale lock with dead PID) **and a no-side-effects assertion (status changes zero repo files, incl. `.manifest`)**; `review_revision` helper: moves on sidecar/style/seed/toolchain/preview change, stable across no-op mtime touches, shared by `status` and `approve`; `adjust` merge preserves unrelated pp3 sections (fixtures from real sidecars), writes `Green=1.0`, `app_adjustments` previous/last_written tracking, `--reset` restores `previous` and never touches diverged (hand-edited) values, crash-reconcile drops ownership on divergence; `crops` suggested/persisted sourcing + `basis` incl. detector-failure fallback; `--review-file` parsing: crop validation against recorded dimensions rejects bad windows before any persistence, audit-string passthrough, `STALE_REVIEW` on revision mismatch; `preview --stem --style` atomic replace (failure keeps prior JPG) + dimension recording + before/after revisions + provenance recording (input hash + content hash, atomic with dims); `stale_previews` detection (both mismatch cases: input-hash drift and swapped-JPG content drift) and refusal on stale previews **for revision-carrying approvals only** (legacy approve unaffected — tested both ways); approve single-snapshot rule (revision + fingerprint from one read); WB bundle all-or-none reset; `ingest --from` stages to temp, hashes the temp (mutating-source safety), dedup/conflict from staged hash, atomic rename, `--delivery-id` recording, flag-less ingest byte-identical legacy recipes, per-file `failed` entries; `run --stem`/`--force` scoping and **plain `run` legacy-behavior regression** (stop states, fingerprint-demotion-as-success unchanged); lock acquisition on every mutating entry point; atomic recipe/manifest writes; **no-flag regression guard: every existing invocation's stdout/stderr and state transitions are identical to today** (modulo the two declared §2 exceptions: lock acquisition, optional provenance/dimension recipe keys).
...
**Round 3** (1 Critical, 6 Major): all applied — `review_revision` now hashes the approval-fingerprint material (styles, seed, toolchain lock included) plus preview content, shared helper for `status`/`approve`, `config/` watched (1); `adjust`/`preview` return before/after revisions with a strict draft-rebase rule (2); `app_adjustments` records `previous`/`last_written` so reset restores rather than deletes and diverged hand edits survive, with crash reconcile (3); `PARTIAL_FAILURE` + `result`-on-failure envelopes for aggregate commands and per-file `ingest.failed` (4); plain `run` keeps exact legacy semantics with a regression test, UI maps pre-approval stop states to "needs review" (5); `status` gets a non-persisting manifest read (today's `manifest.load()` saves on rebuild) + no-side-effects test (6); `ingest --from` hashes the staged temp copy, not the live source (7). Round 3 also confirmed the round-2 lighter dispositions sound (single token: granularity fine once material fixed; re-stat-and-retry: fine once status is side-effect-free).

**Round 4** (2 Critical, 5 Major; confirmed round-3 fixes 4–7 sound) — loop closed at the 4-round cap. Applied: preview provenance recorded atomically with dims, `stale_previews` in status, approve refuses stale previews (1 — closes the round-4 NOT READY rationale, "approval can authorize pixels the reviewed preview didn't show"); approve single-snapshot rule (2); canvas cache keyed on preview content hash (3); staleness evaluation deferred during the stem's own in-flight command (5); white-balance `Setting`/`Temperature`/`Green` owned and restored as one atomic bundle (7). **Deferred with reasons:** (4) a pipeline-verified `rebase_safe` proof — the only writer that can slip between a locked command's endpoints is a non-locked hand edit in a ~2 s window, and its worst case (a wrongly-preserved checkbox draft) is caught pipeline-side by the revision + provenance checks at approve; (6) WAL/journal for adjust's two-file write — both crash orderings fail conservative (ownership is lost, hand-looking values are never deleted; a Reset that declines to reset is acceptable for a single-user app; full WAL machinery is disproportionate).

**Scoped verify of the round-4 fix batch** (rev 4 → 5.1 diff): fixes 2, 3, 5, 7 PASS; fix 1 FAIL (stale definition covered input-hash drift but not a swapped preview JPG) plus two consistency gaps with the compatibility scoping and one additive-only contradiction (legacy preview paths recording provenance). All four corrected in rev 5.2: staleness = input-hash OR content-hash mismatch; stale-preview refusal and its tests scoped to revision-carrying approvals; §2 declares provenance recording as the second explicit additive exception. Loop closed.
