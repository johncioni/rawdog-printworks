...
| `preview --stem S --style Y --json` | Re-render one preview JPG. Renders to a temp file, atomically replaces the target only on success (failure leaves the previous JPG intact). Records in the recipe, atomically with the decoded render dimensions: per-style **preview provenance** — the hash of the rendering-input material the preview was produced from, plus the preview's content hash. A preview whose stored input hash no longer matches current inputs is *stale* (`status.stale_previews`), and `approve` refuses while any style is stale — this is what makes "the pixels you reviewed are the pixels that render" provable rather than assumed. The batch preview path records the same provenance. Without `--stem/--style`, `preview` behaves exactly as today (plus provenance recording). |
