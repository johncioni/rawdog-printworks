...
- F2-MIRROR GAP CLOSED (3212f6c, TDD). The ledger's one-liner does NOT work — as
  ...
  captured idle, killing reconcile; the field had to go non-optional.
...
- CODEX SANDBOX ROOT-CAUSED AND FIXED 2026-08-15 — supersedes "Codex cannot run
  ...
  `codex-swift-sandbox-fix`. EVERY Swift dispatch must carry BOTH flags: `swift
  build/test --disable-sandbox` and `xcodebuild OTHER_SWIFT_FLAGS='$(inherited)
  -disable-sandbox'`. `CoreSimulator`/`DVTFilePathFSEvents` noise is benign.
...
- WT=~/orca/workspaces/rawdog-printworks/plan2-printworks-app (branch
  johncioni/plan2-printworks-app, HEAD b3fcf2a). Ledger: $WT/.superpowers/sdd/
...
- UNDER-LOAD GATE RUNNING in background (25x `swift test` vs 20 spinners on 10
  ...
  `pgrep -f under-load-gate.sh`. At checkpoint: 4/25 GREEN. Re-runnable script:
