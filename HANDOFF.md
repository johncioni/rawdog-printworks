...
## Done
- TASKS 1-9 SHIP; detail in the docs archive. Task 7 took 3 rounds (watcher killed
  by ⌘N/⌘W; a cache key with NO cache; ingest-Retry escalating to `run --force`).
...
- TASK 10 SMOKE PASSED on scratch (`qa/task-10-*.png`): ⌘, opens Settings; a bogus
  repo path shows an inline error and DISABLES Save; a valid one re-enables it;
  a RAW dropped into `Input/` raised "1 new RAW file — Ingest now?" via the
  watcher and removing it cleared the banner. Did NOT click Ingest (22 renders).
...
- I VERIFY EVERY TASK MYSELF: gates by exit code + a mutation per new test.
  NOTE: m11's test fails by HANGING (8min+) not asserting — flagged for review.
...
- LESSONS: greps confirm the letter, READING the intent; a green suite proves what
  is TESTED not what is correct; `open` does NOT relaunch a running app; a mutation
  whose test can HANG must restore the file independently of the test finishing.
